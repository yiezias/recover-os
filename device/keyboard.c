...
/* 键盘中断处理程序 */
static void intr_keyboard_hdl(void) {
	...
	if (break_code) {  // 若是断码break_code(按键弹起时产生的扫描码)
		...
		uint16_t make_code =
			(scancode &=
			 0xff7f);  // 得到其make_code(按键按下时产生的扫描码)
	...
	/* 若为通码,只处理数组中定义的键以及alt_right和ctrl键,全是make_code */
		...
		bool shift =
			false;	// 判断是否与shift组合,用来在一维数组中索引对应的字符
		if ((scancode < 0x0e) || (scancode == 0x29)
			...
			*******************************/
			...
			if (shift_down_last
			    && caps_lock_last) {  // 如果shift和capslock同时按下
				shift = false;
			} else if (
				shift_down_last
				|| caps_lock_last) {  // 如果shift和capslock任意被按下
				shift = true;
			} else {
				shift = false;
			}
		...
		uint8_t index =
			(scancode &=
			 0x00ff);  // 将扫描码的高字节置0,主要是针对高字节是e0的扫描码.
...
void keyboard_init(void) {
	...
	register_intr_handle(0x21, intr_keyboard_hdl);
